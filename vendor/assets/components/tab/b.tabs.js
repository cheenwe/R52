...
!function ($) {
	 ...
	 */
	var constants = {
		closeBtnTemplate : '<i class="navTabsCloseBtn" title="关闭" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">×</span></i>',
		//设置该样式的tab不会被关闭
	...
	bTabs.prototype.init = function(){
		...
		//为关闭图标绑定事件，并适用于动态新增的
		$('ul.nav-tabs',$tabs).on('click','i',function(e){
	...
	bTabs.prototype.addTab = function(id,title,url,loginCheck){
		...
		var openIframe = function(){
			$(content).append('<iframe frameborder="0" scrolling="yes" src="'+url+'"></iframe>');
			// $(content).append('<iframe frameborder="0" scrolling="yes" src="'+url+'"></iframe>');
		};
		//进行登录验证
    ...
    //全屏
    function showFull(){
        var full=document.getElementById("b_tabs");
